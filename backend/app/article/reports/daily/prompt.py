...
3. 含品溢价计算公式：溢价% = (当前最低价 / 成本估值 - 1) × 100，使用原始数据中的算式
...
5. **不得使用"暴涨""狂热""爆发""井喷"等夸张词汇，除非数据中的涨幅极其显著（>50%）**

## 写作要求
1. 文章结构清晰，使用 Markdown 格式输出
2. 分析有依据，结合行业背景给出见解，但见解必须有数据支撑
3. 语言专业流畅，适合数藏爱好者和投资者阅读，避免平铺直叙
4. 善用对比、趋势、排行等维度增强分析价值，但幅度描述须客观
...
请按 **DAILY.md 四段结构** 撰写 **1500-2200 字** 微信公众号文章：

```
# [标题：含日期 + 今日最大亮点，数字必须来自「已核实数据事实」，不夸张]

[开篇 2-3 句，抓住今日最核心看点，直接点明数字或亮点]

## 📅 今日发售日历
[汇总今日所有发行项，特别指出：
  - 含品的「在售总量（live_total）」和「地板价（live_min_price）」实时数据
  - 是否有优先购项目
  - 发行规模（总量、总值）]
...
## 💎 重点藏品解读
[对每件含品逐一分析：
  1. 题材与背景（结合 sku_desc 中的关键信息）
     - 文物字画/历史题材：联系相关历史文化背景扩展介绍
     - 联名/IP题材：介绍该IP的背景
  2. 发行量 & 稀缺性：配比%、发行份数，低配比=高稀缺
  3. 市场现状：在售总量（实时）、地板价（实时）、是否溢价/破发（用 archive_premiums 中的 premium_pct）
  4. 一句客观的参考性结论（含"仅供参考"）]
（无含品数据时：分析发行主力IP的整体发行特征）

## 🏷️ IP与发行商分析
[每个今日发行IP独立小节：

### IP名称
- 发行频次：历史总次数 + 近30天次数 → 活跃度等级（密集/适中/稀缺，取「已核实数据事实」）
- 上次发行时间与作品（来自 last_launch 字段）
- 昨日市场表现：成交笔数 + 市值（来自 market_snapshot.yesterday，无则说明「暂无市场快照」）
- 发行商「XXX」：历史发行规模（总件数）+代表作+旗下合作IP矩阵]

## 📊 昨日行情复盘
[按以下顺序分析：

### 全市场成交概况
  - 昨日 vs 前日成交量变化（使用预计算的成交量日环比）
  - 近7天市值与成交量走势（结合 summaries_7d 数据或折线图）
![市值成交趋势](CHART:market_trend_line)  ← 若可用

### 板块行情
  - 各板块成交量排行（使用 top_planes 数据）
  - 板块涨跌分布分析（使用 plane_census 数据）
![板块涨跌分布](CHART:plane_census)  ← 若可用
![板块成交排行](CHART:plane_deal_rank)  ← 若可用

### 热门藏品 Top10
  - 全局成交量 Top10 逐一分析（使用 hot_archives_top10 数据）
  - 涨跌情况（avg_amount_rate）、地板价（min_amount）
![热门藏品涨跌](CHART:top_archives)  ← 若可用

### 热门IP成交排行
  - 昨日成交最高的IP（使用 ip_analysis 中的 yesterday_deal_count / yesterday_deal_count_rate）
...
⚠️ **重要约束**：
- 标题及所有数字（溢价率/环比/成交量）**必须来自「已核实数据事实」JSON 或「预计算」标注值**
- `live_total`（在售总量）和 `live_min_price`（地板价）来自实时API，比数据库更准确，优先使用
- IP 昨日市场数据来自 MarketIPSnapshot，若无数据直接注明「暂无市场快照」
