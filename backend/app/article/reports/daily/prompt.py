...
6. **涨跌描述须严格对应数据幅度**：
   ...
   - 涨跌幅 20%-50%：用"大幅上涨/下跌"
   - 涨跌幅 >50%：才可用"显著上涨/下跌"
7. **数字单位规范**：正文中所有 ≥10,000 的数量用"万"表示（保留2位小数，去尾零），例如 36,800份 → 3.68万份，2,208,000 → 220.8万；价格仍保留原始精度（如 ¥60.00）
7. **不得使用反问句、感叹句来制造夸张效果**，全文使用陈述句
8. **严禁在正文中暴露任何英文字段名**（例如 `yesterday_deal_count`、`avg_amount_rate`、`hot_ips_top5`、`market_facts` 等），所有数据都必须用中文自然表述（如"成交笔数"、"均价涨跌幅"等）。这些字段名仅供你内部查找数据，不应出现在最终输出中。
9. **严禁出现以下"自我说明"性句式**："其余IP未在已核实数据事实中提供..."、"由于数据缺失..."、"无法构成完整..."、"故无法..."。当数据不足时，**直接按现有数量陈述**（哪怕只有 2 条也直接列出，标题改为"成交IP排行"无需"Top5"，绝不解释为什么没凑够5个）
...
## 微信公众号格式规范
- 文章标题（第一行 `# 标题`）：格式严格为「数藏日报·MM-DD｜核心摘要」，**总长度必须 ≤ 32 字（含标点）**，**禁止使用问号、感叹号、省略号**
  - 字数分配：「数藏日报·」5字 + 「MM-DD」5字 + 「｜」1字 = 固定11字，核心摘要最多**21字**
  - 摘要部分只写最核心的一组数据，例如「13款·12.8万份·¥584万」（12字）或「21款，鲸探发行节」（9字）
  - 正确示例：`# 数藏日报·04-21｜13款·12.8万份·¥584万`（共26字 ✓）
  - 错误示例：`# 数藏日报·2026-04-21｜今日共发行13款数字藏品，总发行量12.8万份`（超出32字 ✗）
- 一级章节用 `## 一级标题`，二级用 `### 二级标题`
...
- `![分类Top5涨跌](CHART:top_archives)` — 各分类 Top 5 藏品均价涨跌幅对比图
- `![热门藏品Top10](CHART:hot_archives_top10)` — 昨日全局 Top 10 热门藏品综合卡片图（成交量×均价涨跌×地板价）
- `![市值成交趋势](CHART:market_trend_line)` — 近7天全市场市值 & 成交量双轴折线图
- `![核心藏品市值走势](CHART:core_plane_market_line)` — 近7天鲸探50 & 禁出195总市值折线图
- `![IP成交排行](CHART:ip_deal_rank)` — 昨日热门 IP 成交 Top 5 蝴蝶图（左成交额、右成交笔数）
...
def build_analysis_prompt(data: dict) -> str:
    ...
    for ipn, ip_data in (data.get("ip_deep_analysis") or {}).items():
        ...
        r1y = ip_data.get("recent_1y_launches", 0)
        fans = ip_data.get("fans_count", 0)
        ip_activity += (
            f"  - {ipn}：历史发行{total}次，近一年（不含今日）{r1y}次"
    ...
    # 热门IP Top5（昨日全市场成交量Top5，与今日是否发行无关）
    hot_ips_block = ""
    hot_ips = (market.get("hot_ips_top5") or [])[:5]
    for h in hot_ips:
        rate = h.get("deal_count_rate")
        rate_str = f"{rate:+.1f}%" if rate is not None else "N/A"
        ma = h.get("market_amount")
        ma_str = f"¥{ma:,.0f}" if ma else "N/A"
        hot_ips_block += (
            f"  {h.get('rank','-')}. 【{h.get('ip_name','')}】"
            f"成交{h.get('deal_count',0):,}笔（环比{rate_str}），市值{ma_str}\n"
        )
...
{{
  ...
  "ip_analysis": [
    {{
      ...
      "recent_1y_launches": <近一年次数>,
      "fans_count": <粉丝数或null>,
      "today_count": <今日该IP发行项数>,
      "last_launch_time": "<上次发行日期，无则null>",
      "last_launch_name": "<上次发行名，无则null>",
  ...
  "hot_ips_top5": [
    {{
      ...
      "ip_name": "<IP名>",
      "deal_count": <昨日成交笔数>,
      "deal_count_rate": <成交笔数环比%或null>,
      "market_amount": <昨日市值或null>,
      "market_amount_rate": <市值环比%或null>
    }}
  ]
}}"""
...
def build_daily_prompt(data: dict, available_charts: list[str]) -> str:
    ...
    for ipn, ip_data in (data.get("ip_deep_analysis") or {}).items():
        ...
        ip_analysis_block += f"  - 近一年发行次数（不含今日）：{ip_data.get('recent_1y_launches', 0)}次\n"
...
### IP名称
- 发行频次：历史总次数 + 近一年次数（客观陈述，不做活跃度评级）
- 上次发行时间与作品（取 last_launch_time/last_launch_name）；**若两字段任一为 null 或缺失，整行直接跳过，不写"暂无"等说明**
- 昨日市场表现：成交笔数 + 市值变化（取 yesterday_deal_count/yesterday_deal_count_rate）；**若该IP无昨日快照数据，直接跳过此行，不做任何说明**
...
![市值成交趋势](CHART:market_trend_line)  ← 若可用
    - 核心藏品指标：补充分析近7天「鲸探50 / 禁出195」总市值变化
![核心藏品市值走势](CHART:core_plane_market_line)  ← 若可用
...
### 热门IP成交Top5
  - 直接取「已核实数据事实」hot_ips_top5（已按成交量排序，与今日是否发行无关，是全市场排名）
  - 逐一陈述：排名 + IP名 + 成交笔数 + 环比变化 + 昨日市值
  - 简评头部 IP 集中度：Top5 合计成交笔数占全市场（market_facts.yesterday_deal_count）的比例
  - 若 hot_ips_top5 为空数组，整段「热门IP成交Top5」省略
...
def _build_market_block(market: dict) -> str:
    ...
    # ── 近7天核心藏品板块总市值（鲸探50 / 禁出195）──
    core_7d = market.get("core_plane_market_values_7d") or []
    if core_7d:
        lines.append("\n### 近7天核心藏品板块总市值")
        lines.append("| 日期 | 鲸探50总市值 | 禁出195总市值 |")
        lines.append("|------|-------------|---------------|")
        for s in core_7d:
            j50 = s.get("jingtan50_market_value")
            rel = s.get("restricted_relics_market_value")
            j50_s = f"¥{j50/1e8:.2f}亿" if j50 is not None else "—"
            rel_s = f"¥{rel/1e8:.2f}亿" if rel is not None else "—"
            lines.append(f"| {s['stat_date']} | {j50_s} | {rel_s} |")
