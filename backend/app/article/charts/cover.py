"""封面图生成 — 微信公众号首图（900×383 逻辑尺寸，2× 输出）。
...
# 输出分辨率倍数（2 = 2× 清晰度，最终图片为 1800×766）
SCALE = 2
...
# ── 颜色 ─────────────────────────────────────────────────
GRAD_LEFT  = (10, 20, 60)
GRAD_RIGHT = (28, 54, 120)
ACCENT     = (0, 210, 255)
TEXT_WHITE = (255, 255, 255)
TEXT_LIGHT = (180, 210, 255)
DIVIDER    = (60, 100, 180)
LOGO_BG    = (20, 38, 90, 200)
...
def _draw_accent_line(draw: ImageDraw.ImageDraw, x: int, y0: int, y1: int) -> None:
    """标题左侧青蓝竖线装饰。"""
    draw.rectangle([x, y0, x + 5 * SCALE, y1], fill=ACCENT)


# 列表模式安全区：微信从中心裁取 H×H 正方形
_LIST_CROP_X0 = (W - H) // 2
_LIST_CROP_CX = W // 2
_MASCOT_CENTER_X = 810 * SCALE
...
def _place_source_logos(canvas: Image.Image, draw: ImageDraw.ImageDraw) -> None:
    ...
    x_cursor = _LIST_CROP_X0 + 28 * SCALE
    y_center  = H - 20 * SCALE
    small_font = _load_font(13 * SCALE)

    for i, (logo_path, label) in enumerate(logos_info):
        if i > 0:
            dot = "  ·  "
            draw.text((x_cursor, y_center - 8 * SCALE), dot, font=small_font, fill=TEXT_LIGHT)
        ...
        if label:
            ...
            draw.text((x_cursor, y_center - 8 * SCALE), label, font=small_font, fill=TEXT_LIGHT)
            x_cursor += bb[2] - bb[0]
...
def generate_cover(
    ...
    for x in range(fade_w):
        alpha = int(120 * (x / fade_w))
        mask_draw.line([(x, 0), (x, H)], fill=(8, 16, 50, alpha))
    mask_draw.rectangle([fade_w, 0, 620 * SCALE, H], fill=(8, 16, 50, 130))
    for x in range(620 * SCALE, 720 * SCALE):
        alpha = int(130 * (1 - (x - 620 * SCALE) / (100 * SCALE)))
        mask_draw.line([(x, 0), (x, H)], fill=(8, 16, 50, alpha))
    canvas = Image.alpha_composite(canvas, mask)
    draw = ImageDraw.Draw(canvas, "RGBA")

    # ⑤ 字体 & 文字左对齐锚点
    TEXT_X = _LIST_CROP_X0 + 28 * SCALE
    parts = title.split("·") if "·" in title else [title, ""]
    type_text = parts[0].strip()
    date_text = parts[1].strip() if len(parts) > 1 else ""

    font_type = _load_font(34 * SCALE, bold=True)
    font_date = _load_font(56 * SCALE, bold=True)
    font_sub  = _load_font(19 * SCALE)

    # ⑥ 竖线装饰
    _draw_accent_line(draw, TEXT_X - 12 * SCALE, 82 * SCALE, 216 * SCALE)

    # 主标题两行
    draw.text((TEXT_X, 85 * SCALE), type_text, font=font_type, fill=TEXT_WHITE)
    if date_text:
        draw.text((TEXT_X, 128 * SCALE), date_text, font=font_date, fill=ACCENT)

    # ⑦ 分隔线
    draw.rectangle([TEXT_X, 228 * SCALE, TEXT_X + 270 * SCALE, 230 * SCALE], fill=DIVIDER)
