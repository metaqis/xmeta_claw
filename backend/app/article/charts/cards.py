...
def generate_launch_grid(
    ...
    CARD_W     = W - 2 * H_PAD
    right_avail = CARD_W - LEFT_W - 1 - BODY_PAD - 10
    ...
    def _badge(draw, x, y, text, ft, bg, fg, ph=6, pv=3, r=4):
    ...
    def _card_h(launch: dict) -> int:
        ...
        l_h  = BODY_PAD + COVER_SZ + 8 + 22 + BODY_PAD
        r_h  = BODY_PAD
        r_h += 24 * 2 + 8
        r_h += 20 + 8
    ...
    draw.rectangle([0, 0, 6, GLOBAL_H], fill=C_ACCENT)
    draw.rectangle([0, GLOBAL_H - 4, W, GLOBAL_H], fill=C_ACCENT)
    ...
    draw.text((H_PAD + 12, 10), date_str,       fill=(130, 165, 225), font=FT[13])
    draw.text((H_PAD + 12, 28), "当日发行总览",  fill=C_WHITE,         font=FT[22])

    stats_str = f"共 {n} 项  ·  总量 {total_count:,} 份  ·  总价值 ¥{total_val:,.0f}"
    sw = _tw(draw, stats_str, FT[13])
    draw.text((W - sw - H_PAD - 12, 34), stats_str, fill=(150, 185, 240), font=FT[13])
    ...
    for launch in launches:
        ...
        # 软阴影
        draw.rounded_rectangle([cx + 4, cy + 4, cx + CARD_W + 4, cy + ch + 4],
                                radius=CARD_R, fill=C_SHADOW1)
        draw.rounded_rectangle([cx + 2, cy + 2, cx + CARD_W + 2, cy + ch + 2],
        ...
        draw.rectangle([cx + 12, cy + 11, cx + 15, cy + STRIP_H - 11], fill=(200, 225, 255))
        draw.text((cx + 22, cy + (STRIP_H - 15) // 2), ip_name, fill=C_WHITE, font=FT[15])
        ...
        if tag_str:
            ...
            draw.text((cx + CARD_W - tw_ - 14, cy + (STRIP_H - 13) // 2),
        ...
        if cover:
            _round_paste(canvas, cover, (img_x, img_y), radius=8)
            draw.rounded_rectangle([img_x - 1, img_y - 1, img_x + COVER_SZ, img_y + COVER_SZ],
                                   radius=8, outline=C_BORDER)
        else:
            draw.rounded_rectangle([img_x, img_y, img_x + COVER_SZ - 1, img_y + COVER_SZ - 1],
                                   radius=8, fill=(228, 233, 248), outline=C_BORDER)
            draw.text((img_x + 52, img_y + 82), "暂无图片", fill=C_LIGHT, font=FT[12])
        ...
        pill_y  = img_y + COVER_SZ + 7
        draw.rounded_rectangle([pill_x, pill_y, pill_x + pill_w, pill_y + 22],
                               radius=11, fill=C_BORDER)
        draw.text((pill_x + 10, pill_y + 4), cnt_str, fill=C_DARK, font=FT[13])
        ...
        for i, line in enumerate(name_lines):
            draw.text((rx, ry + i * 25), line, fill=C_DARK, font=FT[17])
        ry += len(name_lines) * 25 + 8
        ...
        if contain:
            ...
            ry += 16
            draw.line([rx, ry, rx + right_avail, ry], fill=C_BORDER, width=1)
            ry += 8
            ...
            for ci, ca in enumerate(contain[: ARCH_COLS * 2]):
                ...
                if pct:
                    ...
                    pw  = ptw + 8
                    px0 = ax_ + arch_cell_w - pw - 2
                    draw.rounded_rectangle([px0, ay_ + 2, px0 + pw - 1, ay_ + 17],
                                           radius=4, fill=C_ACCENT)
                    draw.text((px0 + 4, ay_ + 3), ps, fill=C_WHITE, font=FT[10])

                a_name  = ca.get("archive_name") or ""
                a_lines = _wrap(a_name, arch_cell_w - 4, FT[11])[:1]
                ...
                if mp:
                    ...
                    draw.text((mx,         ay_ + THUMB_SZ + 20), price_txt, fill=C_GREEN, font=FT[11])
                    if deal_txt:
                        draw.text((mx + ptw_, ay_ + THUMB_SZ + 20), deal_txt, fill=C_MID, font=FT[11])
                else:
                    sn = ca.get("sales_num") or 0
                    if sn:
                        sn_txt = f"{sn:,}份"
                        snw = _tw(draw, sn_txt, FT[11])
                        draw.text((ax_ + max(0, (arch_cell_w - snw) // 2), ay_ + THUMB_SZ + 20),
